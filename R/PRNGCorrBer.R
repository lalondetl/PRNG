...
#'
#' This function produces pseudo-random numbers for both predictors and responses for correlated (longitudinal) data.  The responses are generated as binary (Bernoulli) data.  The predictors can be either continuous or binary, and can include both time-independent and time-dependent covariates.  Time-dependent covariates can be controlled as either Type I, Type II, Type III, or Type IV time-dependent covariates.  The function returns a response vector, a design matrix for the time-dependent covariates (with intercept column), and a design matrix for the time-independent covariates.  
...
# NUMBER OF PREDICTORS, EXCLUDE INTERCEPT #
q = length(beta) - 1
...
for(i in 1:S)
{
	...
	for(t in 1:T_i)
	{
		...
		if(t == 1)
		{
			...
			# SUCCESS PROBABILITIES (mu) AND OUTCOMES (y) #
		...
		else # t > 1 #
		{
			...
			for(j in 1:q)
			{
				...
				if(dataTypes[j] == 'c')
				{
					...
					{
						px[t,j] = (0+rhoyx*log(((mu[t-1]^(y[t-1]))*(1-mu[t-1])^(1-y[t-1]))/(1-(mu[t-1]^(y[t-1]))*(1-mu[t-1])^(1-y[t-1]))))
