...
#' Pseudo-Random Number Generation of Correlated Count (Poisson) Outcomes
#'
#' This function produces pseudo-random numbers for both predictors and responses for correlated (longitudinal) data.  The responses are generated as count (Poisson) data.  The predictors can be either continuous or binary, and can include both time-independent and time-dependent covariates.  Time-dependent covariates can be controlled as either Type I, Type II, Type III, or Type IV time-dependent covariates.  The function returns a response vector, a design matric for the time-dependent covariates (with intercept column), and a design matrix for the time-independent covariates.  
...
PRNGCorrPoi = function(seed,S,Tvec,rhoyy,rhoxy,rhoyx,TDCTypes,dataTypes,beta,pred){
...
for(i in 1:S)
{
	...
	for(t in 1:T_i)
	{
		...
		if(t == 1)
		{
			...
			#store means (mu) and outcomes (y)
			mu[t]<-exp(X[t,]%*%beta+0.2*X_TIC[i,1])
			y[t]<-rpois(1,mu[t])
		}
		...
		else # t > 1 #
		{
			...
			for(j in 1:q)
			{
				if(dataTypes[j] == 'c')
				{
					...
					{
						px[t,j] = (beta_x+rhoyx*log((ppois(y[t-1],mu[t-1]))/(1-(ppois(y[t-1],mu[t-1])))))
						X[t,(j+1)] = rnorm(1,px[t,j],pred[j])
					}
				}
				else # DATA TYPE == 'b' #
				{
					...
					{
						eta_j = log(pred[j]/(1-pred[j]))+rhoyx*log((ppois(y[t-1],mu[t-1]))/(1-(ppois(y[t-1],mu[t-1]))))
			...
			eta_t = X[t,]%*%beta+rhoxy*(x_prev)+rhoyy*log((ppois(y[t-1],mu[t-1]))/(1-(ppois(y[t-1],mu[t-1]))))
			mu[t]<-exp(eta_t+0.2*X_TIC[i,1])
