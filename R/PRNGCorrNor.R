		...
		else # t > 1 #
		{
			...
			for(j in 1:q)
			{
				if(dataTypes[j] == 'c')
				{
					...
					{
						px[t,j] = (beta_x+rhoyx*log((pnorm(y[t-1],mu[t-1],sigma))/(1-(pnorm(y[t-1],mu[t-1],sigma)))))
						X[t,(j+1)] = rnorm(1,px[t,j],pred[j])
					}
				}
				else # DATA TYPE == 'b' #
			...
			eta_t = X[t,]%*%beta+0.2*X_TIC[i,1]+rhoxy*(x_prev)+rhoyy*log((pnorm(y[t-1],mu[t-1],sigma))/(1-(pnorm(y[t-1],mu[t-1],sigma))))
			mu[t]<-eta_t
